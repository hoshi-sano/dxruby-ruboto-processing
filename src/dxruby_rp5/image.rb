module DXRubyRP5
  class Image
    ...
    def self.load(filename, x = nil, y = nil, width = nil, height = nil)
      image = self.new(0, 0)
      surface = $sketch.load_image(filename)
      surface.format = $sketch.class::ARGB
    ...
    def self.load_tiles(filename, xcount, ycount, switch = true)
      ...
      ycount.times do |y|
        xcount.times do |x|
          image = new(0, 0)
          s = $sketch.create_image(width, height, $sketch.class::ARGB)
    ...
    def initialize(width, height, _color = [0, 0, 0, 0])
      ...
      @color = $sketch.color(*_color)

      if width == 0 && height == 0
        return
      end

      @_surface = $sketch.create_image(width, height, $sketch.class::ARGB)
    ...
    def []=(x, y, _color)
      _color = to_rp5_color(_color)
      return @_surface.set(x, y, $sketch.color(*_color))
    end

    def set_color_key(_color)
      r, g, b = *_color[-3..-1]
      alpha = $sketch.color(r, g, b, 0)
